...
export function Breadcrumbs({ section, group, subgroup }: Props) {
  const showNested = !FLAT_SECTIONS.has(section);
  ...
  const crumbs = [
    ...
    showNested && group && { label: group, href: groupHref },
    showNested && subgroup && subgroup !== group && { label: subgroup, href: subgroupHref },
  ].filter(Boolean) as { label: string; href: string | null }[];

  return (
    <nav aria-label="Breadcrumb" data-print-hide className="hidden md:flex items-center gap-1 text-xs text-muted-foreground mb-4 flex-wrap">
      {crumbs.map((crumb, i) => (
