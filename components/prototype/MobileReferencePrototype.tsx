...
type Tab = "home" | "codes" | "vademecum" | "map";
type CatalogItem = {
  id: string;
  title: string;
...
const QUICK_ACTIONS = [
  { label: "Procedimientos", hint: "234 disponibles", icon: FileText, tab: "home" as Tab, query: "" },
  { label: "Vademécum", hint: "Dosis y vías", icon: Pill, tab: "vademecum" as Tab, query: "" },
  { label: "Códigos", hint: "Radio y patología", icon: Radio, tab: "codes" as Tab, query: "" },
  { label: "Abreviaturas", hint: "Consulta rápida", icon: CaseSensitive, tab: "home" as Tab, query: "AAS" },
];
...
function BottomNav({ tab, onChange }: { tab: Tab; onChange: (tab: Tab) => void }) {
  const items = [{ id: "home" as Tab, label: "Inicio", icon: Home }, { id: "codes" as Tab, label: "Códigos", icon: Radio }, { id: "vademecum" as Tab, label: "Vademécum", icon: Pill }, { id: "map" as Tab, label: "Mapa", icon: MapPinned }];
...
function SharedTabScreen({ tab, query, setQuery, procedures, saved, toggleSaved, openItem, version }: { tab: Tab; query: string; setQuery: (value: string) => void; procedures: ProcedureNavMeta[]; saved: string[]; toggleSaved: (id: string) => void; openItem: (title: string, kind: string) => void; version: string }) {
  ...
  const scopedCatalog = tab === "codes" ? SAMPLE_CODES : tab === "vademecum" ? SAMPLE_DRUGS : catalog;
  const filtered = scopedCatalog.filter((item) => `${item.title} ${item.detail} ${item.id}`.toLowerCase().includes(query.toLowerCase())).slice(0, 8);
  if (tab === "map") return <><SectionHeading eyebrow="Entrada local" title="Mapa" /><MapPanel /></>;
  const sectionTitle = tab === "codes" ? "Códigos" : tab === "vademecum" ? "Vademécum" : "Búsqueda global";
  const sectionEyebrow = tab === "codes" ? "Comunicación y patología" : tab === "vademecum" ? "Dosis y vías" : "Consulta local";
  return <><SectionHeading eyebrow={sectionEyebrow} title={sectionTitle} /><SearchField value={query} onChange={setQuery} placeholder={tab === "codes" ? "Buscar código o descripción" : tab === "vademecum" ? "Buscar fármaco o nombre comercial" : "Buscar procedimiento, fármaco o código"} /><div className="mt-4 flex gap-2 overflow-x-auto pb-1">{(tab === "home" ? ["Todo", "Procedimientos", "Fármacos", "Códigos", "Abreviaturas"] : tab === "codes" ? ["Todos", "Incidente", "SVA", "SVB"] : ["Todos", "Fármacos", "Perfusiones", "Fluidos"]).map((label, index) => <span key={label} className={`rounded-full px-3 py-1.5 text-xs font-semibold ${index === 0 ? "bg-red-500 text-white" : "bg-muted text-muted-foreground"}`}>{label}</span>)}</div><div className="mt-5 flex items-center justify-between"><p className="text-xs font-semibold text-muted-foreground">{query ? `${filtered.length} resultados locales` : "Sugerencias para empezar"}</p><button type="button" className="flex items-center gap-1 text-xs font-semibold text-red-500"><SlidersHorizontal className="h-3.5 w-3.5" /> Filtrar</button></div><div className="mt-2 space-y-2">{filtered.map((item) => item.kind === "Procedimiento" ? <ProcedureRow key={item.id} procedure={procedures.find((procedure) => procedure.id === item.id)!} onOpen={() => openItem(item.title, item.kind)} saved={saved.includes(item.id)} onToggleSaved={() => toggleSaved(item.id)} /> : <CatalogRow key={item.id} item={item} onOpen={() => openItem(item.title, item.kind)} />)}{!filtered.length && <EmptyState title="Sin coincidencias" body="Prueba con otro término o vuelve a Inicio para ver accesos rápidos." icon={<Search className="h-5 w-5" />} />}</div><div className="mt-5"><PackageStatus version={version} /></div></>;
}
...
function VariantA({ tab, setTab, query, setQuery, searchOpen, setSearchOpen, procedures, saved, toggleSaved, openItem, version }: VariantContentProps) {
  return <div className="min-h-dvh bg-[#f8f8f7] text-slate-950 dark:bg-[#0d1015] dark:text-white"><div className="mx-auto flex min-h-dvh w-full max-w-[32rem] flex-col"><header className="bg-slate-950 px-5 pb-6 pt-[calc(1.1rem+env(safe-area-inset-top))] text-white"><div className="flex items-center justify-between"><Brand compact /><button type="button" className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10" aria-label="Ajustes"><Settings2 className="h-4 w-4" /></button></div><div className="mt-7"><p className="text-sm text-slate-400">Buenas tardes, equipo</p><h1 className="mt-1 text-2xl font-bold tracking-tight">¿Qué necesitas<br />consultar ahora?</h1></div><div className="mt-5"><SearchField value={query} onChange={(value) => { setQuery(value); setSearchOpen(true); }} placeholder="Buscar en todo el manual" /></div></header><main className="flex-1 space-y-7 px-4 py-5 pb-6">{tab === "home" && !searchOpen ? <><section><SectionHeading eyebrow="Accesos directos" title="Consulta por área" /><QuickActions onAction={(nextTab, nextQuery) => { setTab(nextTab); setQuery(nextQuery); setSearchOpen(nextTab === "home"); }} /></section><section><SectionHeading eyebrow="Continúa donde lo dejaste" title="Últimas consultas" action="Ver todo" /><div className="space-y-2"><ProcedureRow procedure={procedures[0]} onOpen={() => openItem(procedures[0].title, "Procedimiento")} saved={saved.includes(procedures[0].id)} onToggleSaved={() => toggleSaved(procedures[0].id)} /><ProcedureRow procedure={procedures[1] ?? procedures[0]} onOpen={() => openItem((procedures[1] ?? procedures[0]).title, "Procedimiento")} saved={saved.includes((procedures[1] ?? procedures[0]).id)} onToggleSaved={() => toggleSaved((procedures[1] ?? procedures[0]).id)} /></div></section><PackageStatus version={version} /><div className="flex items-start gap-2 px-1 text-[10px] leading-4 text-muted-foreground"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Referencia no oficial. Comprobar siempre la versión oficial y el contexto asistencial.</span></div></> : <SharedTabScreen tab="home" query={query} setQuery={setQuery} procedures={procedures} saved={saved} toggleSaved={toggleSaved} openItem={openItem} version={version} />}</main><BottomNav tab={tab} onChange={(nextTab) => { setTab(nextTab); setSearchOpen(false); }} /></div></div>;
}

function VariantB({ tab, setTab, query, setQuery, searchOpen, setSearchOpen, procedures, saved, toggleSaved, openItem, version }: VariantContentProps) {
  return <div className="min-h-dvh bg-[#eef2f5] text-slate-950 dark:bg-[#0b1016] dark:text-white"><div className="mx-auto flex min-h-dvh w-full max-w-[32rem] flex-col"><header className="border-b border-slate-200 bg-white px-4 pb-4 pt-[calc(1rem+env(safe-area-inset-top))] dark:border-slate-800 dark:bg-slate-950"><div className="flex items-center justify-between"><Brand compact /><button type="button" className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800" aria-label="Abrir menú"><Menu className="h-5 w-5" /></button></div>{tab === "home" || searchOpen ? <div className="mt-5"><div className="mb-2 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[0.16em] text-red-500">Modo respuesta</p><span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> offline listo</span></div><SearchField value={query} onChange={(value) => { setQuery(value); setSearchOpen(true); }} placeholder="ID, síntoma, fármaco o código" /></div> : null}</header><main className="flex-1 px-4 py-4 pb-6">{tab === "home" && !searchOpen ? <><div className="mb-5 rounded-3xl bg-red-600 p-5 text-white shadow-xl shadow-red-600/20"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-red-100">Acción inmediata</p><h1 className="mt-2 max-w-[15rem] text-2xl font-bold leading-tight">Accede al manual en un toque.</h1></div><Sparkles className="h-6 w-6 text-red-200" /></div><button type="button" onClick={() => setSearchOpen(true)} className="mt-5 flex min-h-12 w-full items-center justify-between rounded-2xl bg-white px-4 text-left text-sm font-bold text-red-700">Abrir búsqueda global <ArrowRight className="h-4 w-4" /></button></div><div className="mb-5 grid grid-cols-4 gap-2"><QuickActions horizontal onAction={(nextTab, nextQuery) => { setTab(nextTab); setQuery(nextQuery); setSearchOpen(nextTab === "home"); }} /></div><section><SectionHeading eyebrow="Retomar" title="Actividad reciente" action="Limpiar" /><div className="divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white px-4 dark:divide-slate-800 dark:border-slate-800 dark:bg-slate-950"><ProcedureRow procedure={procedures[0]} onOpen={() => openItem(procedures[0].title, "Procedimiento")} saved={saved.includes(procedures[0].id)} onToggleSaved={() => toggleSaved(procedures[0].id)} dense /><ProcedureRow procedure={procedures[2] ?? procedures[0]} onOpen={() => openItem((procedures[2] ?? procedures[0]).title, "Procedimiento")} saved={saved.includes((procedures[2] ?? procedures[0]).id)} onToggleSaved={() => toggleSaved((procedures[2] ?? procedures[0]).id)} dense /></div></section></> : <SharedTabScreen tab="home" query={query} setQuery={setQuery} procedures={procedures} saved={saved} toggleSaved={toggleSaved} openItem={openItem} version={version} />}</main><BottomNav tab={tab} onChange={(nextTab) => { setTab(nextTab); setSearchOpen(false); }} /></div></div>;
}

function VariantC({ tab, setTab, query, setQuery, searchOpen, setSearchOpen, procedures, saved, toggleSaved, openItem, version }: VariantContentProps) {
  const nav = [{ id: "home" as Tab, label: "Inicio", icon: Home }, { id: "codes" as Tab, label: "Códigos", icon: Radio }, { id: "vademecum" as Tab, label: "Vademécum", icon: Pill }, { id: "map" as Tab, label: "Mapa", icon: MapPinned }];
  return <div className="min-h-dvh bg-[#fbfaf8] text-stone-950 dark:bg-[#111111] dark:text-white"><div className="mx-auto grid min-h-dvh w-full max-w-[64rem] md:grid-cols-[14rem_1fr]"><aside className="hidden border-r border-stone-200 bg-stone-100/70 p-4 dark:border-stone-800 dark:bg-stone-950 md:flex md:flex-col"><Brand compact /><p className="mb-2 mt-10 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Espacios</p><div className="space-y-1">{nav.map(({ id, label, icon: Icon }) => <button type="button" key={id} onClick={() => { setTab(id); setSearchOpen(false); }} className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold ${tab === id ? "bg-white text-red-500 shadow-sm dark:bg-stone-900" : "text-muted-foreground hover:bg-white/70 dark:hover:bg-stone-900"}`}><Icon className="h-4 w-4" />{label}</button>)}</div><div className="mt-auto rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs dark:border-emerald-900/60 dark:bg-emerald-950/30"><p className="font-bold text-emerald-800 dark:text-emerald-300">Paquete local</p><p className="mt-1 text-emerald-700/70 dark:text-emerald-300/70">Validado · {version}</p></div></aside><div className="flex min-w-0 flex-col"><header className="border-b border-stone-200 px-4 pb-4 pt-[calc(1rem+env(safe-area-inset-top))] dark:border-stone-800 md:px-8"><div className="flex items-center justify-between"><div className="md:hidden"><Brand compact /></div><div className="hidden md:block"><p className="text-xs font-bold uppercase tracking-[0.18em] text-red-500">Biblioteca de guardia</p><h1 className="mt-1 text-2xl font-bold">Referencia SAMUR</h1></div><div className="flex items-center gap-2"><button type="button" className="flex h-10 w-10 items-center justify-center rounded-xl border border-border/70" aria-label="Filtrar"><SlidersHorizontal className="h-4 w-4" /></button><button type="button" className="flex h-10 w-10 items-center justify-center rounded-xl border border-border/70 md:hidden" aria-label="Abrir menú"><Menu className="h-4 w-4" /></button></div></div></header><main className="mx-auto w-full max-w-3xl flex-1 px-4 py-5 pb-6 md:px-8 md:py-8">{tab === "home" && !searchOpen ? <><div className="mb-6 grid gap-5 md:grid-cols-[1fr_15rem] md:items-start"><div><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-red-500">Consulta abierta</p><h2 className="max-w-xl text-3xl font-bold leading-tight tracking-tight">Todo el manual, organizado para encontrarlo.</h2><div className="mt-5"><SearchField value={query} onChange={(value) => { setQuery(value); setSearchOpen(true); }} placeholder="Buscar en el manual" /></div></div><PackageStatus version={version} /></div><div className="mb-7"><SectionHeading eyebrow="Colecciones" title="Atajos" /><QuickActions onAction={(nextTab, nextQuery) => { setTab(nextTab); setQuery(nextQuery); setSearchOpen(nextTab === "home"); }} /></div><div className="grid gap-7 md:grid-cols-2"><section><SectionHeading eyebrow="Recientes" title="Continúa" action="Ver todo" /><div className="divide-y divide-border/60">{procedures.slice(0, 3).map((procedure) => <ProcedureRow key={procedure.id} procedure={procedure} onOpen={() => openItem(procedure.title, "Procedimiento")} saved={saved.includes(procedure.id)} onToggleSaved={() => toggleSaved(procedure.id)} dense />)}</div></section><section className="rounded-3xl bg-stone-100 p-5 dark:bg-stone-900"><p className="text-xs font-bold uppercase tracking-[0.18em] text-red-500">Estado de la fuente</p><p className="mt-3 text-lg font-bold">Referencia no oficial</p><p className="mt-2 text-sm leading-6 text-muted-foreground">El paquete muestra la versión y la fecha de validación para que puedas juzgar su vigencia.</p><button type="button" className="mt-4 flex items-center gap-1 text-xs font-bold text-red-500">Ver actualizaciones <ChevronRight className="h-4 w-4" /></button></section></div></> : <SharedTabScreen tab="home" query={query} setQuery={setQuery} procedures={procedures} saved={saved} toggleSaved={toggleSaved} openItem={openItem} version={version} />}</main><div className="md:hidden"><BottomNav tab={tab} onChange={(nextTab) => { setTab(nextTab); setSearchOpen(false); }} /></div></div></div></div>;
}
...
export function MobileReferencePrototype({ procedures, packageVersion }: Props) {
  ...
  const variantProps: VariantContentProps = { tab, setTab, query, setQuery, searchOpen, setSearchOpen, procedures, saved, toggleSaved, openItem: (title, kind) => setSelected({ title, kind }), version: packageVersion };
