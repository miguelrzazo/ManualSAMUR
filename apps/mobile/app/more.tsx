...
export default function MoreScreen() {
  ...
  const favouriteProcedures = snapshot.content.procedures.filter((procedure) => favourites.includes(procedure.routeKey));
  return <Screen onRefresh={() => void refresh()} refreshing={refreshing}>
    ...
    {favouriteProcedures.length ? favouriteProcedures.map((procedure) => <PressableCard key={procedure.routeKey} onPress={() => router.push({ pathname: "/procedure/[id]", params: { id: procedure.routeKey } })}><Text style={commonStyles.label}>{procedure.id} · {procedure.section}</Text><Text style={commonStyles.title}>{procedure.title}</Text></PressableCard>) : <Text style={commonStyles.subtitle}>Guarda fichas con la estrella para encontrarlas aquí.</Text>}
