...
export default function VademecumScreen() {
  ...
  return <Screen>
    ...
    {rows.map((row, index) => { const id = String(row.id ?? row.drugId ?? row.name ?? index); const title = String(row.name ?? row.drug ?? row.activeIngredient ?? row.title ?? "Referencia"); const subtitle = String(row.category ?? row.indication ?? row.presentation ?? row.description ?? ""); const canOpen = typeof row.id === "string" || (tab === "commercialNames" && typeof row.drugId === "string"); return <PressableCard key={id} onPress={() => canOpen ? tab === "drugs" ? router.push({ pathname: "/drug/[id]", params: { id } }) : router.push({ pathname: "/reference/[kind]/[id]", params: { kind: tab, id } }) : undefined} accessibilityLabel={title}><Text style={commonStyles.title}>{title}</Text>{Boolean(subtitle) && <Text style={commonStyles.subtitle} numberOfLines={3}>{subtitle}</Text>}{canOpen && <Text style={styles.open}>Ver ficha</Text>}</PressableCard>; })}
  </Screen>;
}
