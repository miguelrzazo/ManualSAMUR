...
export default function GlobalSearch() {
  ...
  const results = useMemo<Result[]>(() => {
    ...
    const procedures = snapshot.content.procedures.filter(matches).slice(0, 8).map((item) => ({ id: item.id, type: "Procedimiento", title: item.title, subtitle: `${item.id} · ${item.section}`, onPress: () => router.push({ pathname: "/procedure/[id]", params: { id: item.id } }) }));
