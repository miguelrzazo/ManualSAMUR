...
export default function ProcedureDetail() {
  ...
  const procedure = snapshot.content.procedures.find((item) => item.slug === id || item.id === id);
  ...
  useEffect(() => { void readFavourites().then((items) => setFavourite(procedure ? items.includes(procedure.slug) : false)); }, [procedure]);
  if (!procedure) return <Screen><Text style={commonStyles.title}>No se encontró este procedimiento.</Text></Screen>;
  const save = async () => { setFavourite((value) => !value); await toggleFavourite(procedure.slug); void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); };
  ...
  return <Screen scrollRef={scrollRef}>
    ...
    {procedure.related.length > 0 && <View style={{ gap: 8 }}><Text style={commonStyles.title}>Procedimientos relacionados</Text>{procedure.related.map((relatedId) => { const related = snapshot.content.procedures.find((item) => item.id === relatedId); return related ? <PressableCard key={related.slug} onPress={() => router.push({ pathname: "/procedure/[id]", params: { id: related.slug } })}><Text style={commonStyles.label}>{related.id} · {related.section}</Text><Text style={commonStyles.title}>{related.title}</Text></PressableCard> : null; })}</View>}
  </Screen>;
}
