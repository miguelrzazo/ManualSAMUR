...
function SearchBar({ value, onChangeText, onPress }: { value?: string; onChangeText?: (value: string) => void; onPress?: () => void }) {
  return (
    <Pressable onPress={onPress} style={styles.searchBar} accessibilityRole={onChangeText ? "none" : "button"} accessibilityLabel={routeAccessibilityLabels.Buscar} accessibilityHint={onChangeText ? accessibilityHints.search : accessibilityHints.openDetail}>
...
function LocationDetailScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList, "Location">) {
  ...
  return <SafeAreaView style={styles.screen} edges={["top"]}><ScrollView contentContainerStyle={styles.detailContent}>
    ...
    <View style={styles.sourceNotice} accessibilityLabel="Fuente y frescura de la ubicación"><MaterialCommunityIcons name="clock-alert-outline" size={19} color={activePalette.amber} /><Text style={styles.sourceNoticeText}>{locationFreshnessLabel(location, new Date(), policy)}. No es una fuente operativa congelada.</Text></View>
