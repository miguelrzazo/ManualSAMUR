  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top"]}>
      ...
      <View style={styles.filterRow} accessibilityRole="tablist">
        {(["Todo", "Procedimientos", "Fármacos"] as const).map((item) => <Pressable key={item} onPress={() => setFilter(item)} style={[styles.filterChip, filter === item && styles.filterChipActive]} accessibilityRole="tab" accessibilityState={{ selected: filter === item }}><Text style={[styles.filterText, filter === item && styles.filterTextActive]}>{item}</Text></Pressable>)}
      </View>
      <FlatList
        data={[...visibleProcedures.map((item) => ({ kind: "procedure" as const, item })), ...visibleDrugs.map((item) => ({ kind: "drug" as const, item }))]}
...
function CodesScreen({ navigation }: NativeStackScreenProps<RootStackParamList, "Codes">) {
  const { content } = useContent();
  const codes = Object.entries(content.codes).flatMap(([group, items]) => items.slice(0, 12).map((item) => ({ group, item: item as Record<string, unknown> })));
  return <SafeAreaView style={styles.screen} edges={["top"]}><FlatList data={codes} keyExtractor={(item, index) => `${item.group}-${String(item.item.code ?? index)}`} contentContainerStyle={styles.listContent} ListHeaderComponent={<><Pressable onPress={() => navigation.goBack()} accessibilityRole="button" accessibilityLabel="Volver"><MaterialCommunityIcons name="arrow-left" size={24} color={colors.ink} /></Pressable><Text style={styles.pageTitle}>Códigos y claves</Text><Text style={styles.pageKicker}>RADIO · CONSULTA LOCAL</Text></>} renderItem={({ item }) => <View style={styles.codeRow}><Text style={styles.codeValue}>{String(item.item.code ?? "—")}</Text><View style={styles.resourceCopy}><Text style={styles.resourceTitle}>{String(item.item.name ?? item.item.title ?? "Código")}</Text><Text style={styles.resourceMeta}>{item.group} · {String(item.item.description ?? "")}</Text></View></View>} /></SafeAreaView>;
}

function AbbreviationsScreen({ navigation }: NativeStackScreenProps<RootStackParamList, "Abbreviations">) {
  const { content } = useContent();
  const entries = content.abbreviations.flatMap((group) => { const letter = String(group.letter ?? ""); const values = Array.isArray(group.entries) ? group.entries : []; return values.map((entry) => ({ letter, entry: entry as Record<string, unknown> })); });
...
const styles = StyleSheet.create({
  ...
  searchScreenHeader: { paddingHorizontal: spacing.lg, paddingTop: spacing.lg, paddingBottom: spacing.md }, pageTitle: { color: colors.ink, fontSize: 31, fontWeight: "800", letterSpacing: -1 }, pageKicker: { color: colors.red, fontSize: 10, fontWeight: "800", letterSpacing: 1.3, marginTop: 4 }, searchPadding: { paddingHorizontal: spacing.lg },
