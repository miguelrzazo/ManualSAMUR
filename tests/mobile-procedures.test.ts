...
test("classifyMarkdownRows numbers ordered items and separates bullets", () => {
  assert.deepEqual(classifyMarkdownRows(["1. Primer paso.", "1. Segundo paso.", "* Una viñeta.", "1. Nueva lista."]).map((row) => row.kind === "ordered" ? row.ordinal : row.kind), [1, 2, "bullet", 1]);
});
