...
test("platform identity uses the manual's own name without changing the package identifier", () => {
  ...
  assert.equal(config.expo.name, "Manual SAMUR");
  assert.equal(config.expo.ios.infoPlist?.CFBundleDisplayName, "Manual SAMUR");
  assert.equal(config.expo.android?.label, "Manual SAMUR");
